...
| `BackgroundFocusGrantCheck` | Samples whether a host's focus grant to its embedded client lands while a *third* process holds the foreground — the one state the other checks never produce. Prints `grant landed=` plus whether the steal actually took. | **Observational** — no verdict, read across runs |

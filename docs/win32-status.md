...
| Click-to-focus hook install, hit-test, clean unhook                                                                                                                                                                                                                                                                                       | `Win32ClickWatcherTest` under Wine, injected-click delivery included since 2026-09-13; facade path (real injected click → focus returns) real `windows-latest` via `ClickToFocusWin32Check`, 2026-09-03 |
...
| `EmbedSocketWin32#setModal` delivery, end-to-end (`EmbedSocketWin32#listen`'s control channel, a `ControlMessage` MODALITY frame → `EmbedClientWin32#onModalityChanged`)                                                                                                                                                                  | `EmbedSocketWin32Test` (send side) + `EmbedClientWin32Test` (receive side) under Wine + real `windows-latest` via `SocketClientWin32Check` (both halves, one process pair), 2026-09-03 |
...
| `EmbedClientWin32`'s own reparent/focus/resize watching (`announce`/`offer`, `onEmbedded`/`onHostDetached`/`onFocusChanged`/`onResized`) and `requestFocus()` (a `ControlMessage` FOCUS_REQUEST frame on the same control channel `setModal` already uses, the other direction — read by a new `EmbedSocketWin32` control-channel reader) | `EmbedClientWin32Test`/`EmbedSocketWin32Test`/`Win32ConfigureWatcherTest` under Wine + real `windows-latest` via `SocketClientWin32Check`, 2026-09-03                                  |
...
What Wine can't replicate closely enough — foreground-lock policy and
cross-process reparent/DWM behaviour — plus what no single reactor module can
reach
